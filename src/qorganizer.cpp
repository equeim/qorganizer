...
void qOrganizer::addCalendarPageWidgets()
{
  ...
  if(C_COLOR_DATES) {
      ...
      connect(calendar, SIGNAL(clicked(QDate)) , this, SLOT (loadSpecialDays(QDate)));
...
void qOrganizer :: loadSpecialDays(QDate d) {
    loadSpecialDays(d.year(),d.month());
}
